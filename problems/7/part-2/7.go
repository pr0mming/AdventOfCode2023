...
func computeCamelCard(camelCard string) int {
	...
	var jokerCardsAmmount int

	for k, v := range camelCardMap {
		// We keep the Joker cards apart to use it later with the max ammount
		if string(k) != "J" {
	...
	// Fix the "JJJJJ" case
	if len(cardsAmmount) == 0 {
		return 1
	}

	// We extract the highest number from the map to check the Camel Card type
	maxCardAmmount := slices.Max(cardsAmmount) + jokerCardsAmmount
